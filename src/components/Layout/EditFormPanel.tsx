...
/**
 * 오른쪽 패널 — 메타데이터 편집 폼.
 * 단계 3: [컷별] 촬영 날짜 입력(달력+시간 선택기 + 직접 입력) + 이 사진 안전 저장.
 * (롤 공통 구역은 단계 6에서 동작 연결)
 */
interface Props {
  hasImage: boolean;
  dateInput: string;
...
export default function EditFormPanel({
  ...
  return (
    <aside className="flex w-80 shrink-0 flex-col gap-4 overflow-y-auto border-l border-line bg-charcoal p-4">
      {/* 롤 공통 구역 (단계 6에서 활성화) */}
      <section className="rounded-lg border border-line p-3 opacity-60">
        <h2 className="mb-1 text-subtitle font-medium text-paper">롤 공통</h2>
        <p className="mb-3 text-label text-muted">한 통 전체에 동일하게 적용 (단계 6)</p>
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        disabled
        placeholder="—"
        className="w-full rounded border border-line bg-ink px-2 py-1.5 text-body text-paper placeholder:text-muted"
      />
    </label>
  );
}
